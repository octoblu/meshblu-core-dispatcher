  ...
  describe "sending to a device with sendWhitelist", ->
    ...
    describe 'JobManager gets DeliverBroadcastSent job', (done) ->
      ...
      it 'should be a sent messageType', ->
        ...
        expect(metadata.jobType).to.equal 'DeliverBroadcastSent'
        expect(metadata.messageType).to.equal 'broadcast-sent'
        expect(metadata.toUuid).to.equal 'sender-uuid'
        expect(rawData).to.equal JSON.stringify devices: ['*'], payload: 'boo'
